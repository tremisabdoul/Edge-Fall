...
func _physics_process(delta):
	...
	is_wallriding = (
		...
		and
		(!Input.is_action_pressed("jump") or !Input.is_action_just_pressed("jump"))
		and
		(wallride_stamina > 0)
	)
	...
	if is_wallriding:
		...
		#q 	velocity += $Head.global_transform.basis.x * (float($WallRide/Top/Left.is_colliding())-float($WallRide/Top/Right.is_colliding())) * tickspeed
		...
		$Head.rotation.z -= wallriding_dir * .2
		$Head.rotation.z /= 1.1
		$Head.rotation.z += wallriding_dir * .2
	elif is_on_floor():
